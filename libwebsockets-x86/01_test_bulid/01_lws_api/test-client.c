...
static const struct lws_protocols protocols[] = {
	{
		"dumb-increment-protocol",					// 协议的名称
		callback_dumb_increment,					// 对应的回调函数
		0, 20, 0, NULL, 0
	}, {
		"lws-mirror-protocol",
		callback_lws_mirror,
...
int main(int argc, char **argv)
{
	...
	struct lws_context_creation_info info;			// 上下文会话结构体
	struct lws_client_connect_info i;				// 配置客户端连接服务器的相关参数并连接服务器
	...
	info.protocols = protocols;					// 配置相关的各种回调函数来实现与服务器之间的数据交互
	info.gid = (gid_t)-1;
	info.uid = (uid_t)-1;
#if defined(LWS_ROLE_WS) && !defined(LWS_WITHOUT_EXTENSIONS)
...
#endif
	context = lws_create_context(&info);					// 创建会话
	if (context == NULL) {
		...
		if (do_multi) {
			for (n = 0; n < (int)LWS_ARRAY_SIZE(wsi_multi); n++) {
				if (!wsi_multi[n] && ratelimit_connects(&rl_multi[n], 2u)) {
					...
					i.pwsi = &wsi_multi[n];							// store the new wsi here early in the connection process
